...
def create_whisper_model(model_config: Dict[str, Any]) -> Any:
    """
    Factory function for creating WhisperModel instances.
    This function is registered with the ModelCacheManager to create
    models when needed.
    ...
    if not FASTER_WHISPER_AVAILABLE:
        logger.error("faster-whisper not available, cannot create model")
        return None
        
    try:
        # Extract model parameters
        model_size = model_config.get("size", "tiny")
        device = model_config.get("device", "cpu")
        compute_type = model_config.get("compute_type", "int8")
        download_root = model_config.get("download_root", None)
        cpu_threads = model_config.get("cpu_threads", 4)
        
        logger.info(f"Creating WhisperModel({model_size}) on {device} with compute type {compute_type}")
        ...
        # Add download root if provided
        if download_root:
            model_init_params["download_root"] = download_root
            
        # Add CPU-specific parameters
        if device == "cpu":
            model_init_params["cpu_threads"] = cpu_threads
        
        # Create and return the model
        model = WhisperModel(**model_init_params)
        return model
        
    except Exception as e:
        logger.error(f"Error creating WhisperModel: {e}")
        return None

# Register factory with cache manager if available
if MODEL_CACHE_AVAILABLE:
    try:
        cache_manager = get_model_cache_manager()
        cache_manager.register_model_factory("faster-whisper", create_whisper_model)
        logger.info("Registered WhisperModel factory with cache manager")
    except Exception as e:
        logger.warning(f"Failed to register WhisperModel factory: {e}")
...
class FasterWhisperSTT:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.use_model_cache = self.model_config.get('use_model_cache', True)
        
        # Internal state
        self.model = None
        self.model_release_func = lambda: None  # Default no-op release function
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Set up model configuration for initialization or cache retrieval
            model_config = {
                "size": self.model_size,
                "device": device,
                "compute_type": compute_type,
                "download_root": os.path.dirname(self.model_path) if "/" in self.model_path else None,
                "cpu_threads": self.cpu_threads if device == "cpu" else None,
                "language": self.language,
                "type": "faster-whisper"
            }
            
            # Try to get model from cache if available
            if MODEL_CACHE_AVAILABLE:
                try:
                    logger.info("Attempting to get model from cache")
                    cache_manager = get_model_cache_manager()
                    model, release_func = cache_manager.get_model("faster-whisper", model_config)
                    
                    if model is not None:
                        self.model = model
                        self.model_release_func = release_func
                        logger.info(f"Retrieved model from cache: {self.model_size}")
                    else:
                        # Cache miss - create a new model directly
                        logger.info("Model not in cache, creating new instance")
                        self.model = create_whisper_model(model_config)
                        # No release function needed as we're not tracking this in the cache
                        self.model_release_func = lambda: None
                        
                except Exception as cache_error:
                    logger.warning(f"Error using model cache: {cache_error}, falling back to direct initialization")
                    # Fall back to direct initialization
                    model_init_params = {k: v for k, v in model_config.items() if k != "type" and v is not None}
                    self.model = WhisperModel(**model_init_params)
                    self.model_release_func = lambda: None
            else:
                # Cache not available, initialize directly
                logger.info("Model cache not available, creating model directly")
                model_init_params = {
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'performance': {
                ...
                'model_load_time': self.metrics.get('model_load_time', 0)
            },
            'optimizations': {
            ...
            'caching': {
                'enabled': self.use_model_cache and MODEL_CACHE_AVAILABLE,
        ...
        # Add model cache manager status if available
        if MODEL_CACHE_AVAILABLE and self.use_model_cache:
            try:
                cache_manager = get_model_cache_manager()
                cache_status = cache_manager.get_status()
                status['cache_manager'] = {
                    'cached_models_count': cache_status.get('cache_size', 0),
                    'max_cache_size': cache_status.get('max_cache_size', 0),
                    'memory_usage': cache_status.get('memory_usage', 0),
                    'is_jetson': cache_status.get('is_jetson', False)
                }
            except Exception as e:
                logger.debug(f"Failed to get cache manager status: {e}")
    ...
    def shutdown(self) -> bool:
        ...
        """
        try:
            # Call model release function if available (to decrement reference count in cache)
            if hasattr(self, 'model_release_func') and self.model_release_func:
                try:
                    self.model_release_func()
                    logger.info("Released model back to cache")
                except Exception as release_error:
                    logger.warning(f"Error releasing model to cache: {release_error}")
                
            # Set model reference to None
            self.model = None
